...
**Outcome (fill in after Phase 1a closes):** Cap NOT hit. Draft generated 2026-07-13T16:44:08Z via `gemini-3.5-flash` (36 nodes, 51 edges). Manual correction (recorded in `scripts/_apply_graph_corrections.py`) applied three targeted fixes — (1) removed section-uniqueness violations where premium clubs re-listed concourse sections, (2) changed the sole `coaches_club` edge from `stairs_only` to `elevator` to restore accessibility-subgraph connectivity, (3) filled in missing lower/mezzanine east-arc sections — and `make verify-graph` passed at ~17:04Z, roughly 20 minutes into the cap. Final graph: **36 nodes, 51 edges**, within the 35–45 target. Fallback graph was not needed. **Post-hoc patch (same day, during Phase 1 intent validation):** the corrected draft had zero `stairs_only` edges, which would have made Entry #7's stairs-warning safety check and Entry #9's accessibility-filtered Dijkstra observationally indistinguishable from unfiltered routing — the flagship GenAI-justification query in Entry #4 (wheelchair fan) would have produced identical output with or without the constraint. Added 3 `stairs_only` edges as parallel connections alongside existing elevator/ramp edges (100↔200 north/west, 200↔300 north), each 2 minutes vs. the accessible alternative's 4–5 minutes, mirroring how real venues work (stairs are faster for the general flow; the accessible option stays available). Final graph is now **36 nodes, 54 edges**.
