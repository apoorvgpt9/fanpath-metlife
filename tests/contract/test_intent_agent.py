...
def test_navigation_resolved(mock_pro, small_graph, fan):
    ...
    result = parse_navigation_request("get me from A to G", fan, [], small_graph)
...
@patch("app.agents.intent.pro")
def test_navigation_ambiguous(mock_pro, small_graph, fan):
    mock_pro.return_value = _mock_client(
        json.dumps(
            {
                "type": "ambiguous",
    ...
    result = parse_navigation_request("get me to the junction", fan, [], small_graph)
    assert isinstance(result, AmbiguousRequest)
    assert set(result.candidates) == {"b", "c"}


@patch("app.agents.intent.pro")
def test_navigation_unresolvable(mock_pro, small_graph, fan):
    mock_pro.return_value = _mock_client(
        json.dumps({"type": "unresolvable", "reason": "off-topic query"})
    )
    result = parse_navigation_request("what's the score?", fan, [], small_graph)
    assert isinstance(result, UnresolvableRequest)
    assert "off-topic" in result.reason


@patch("app.agents.intent.pro")
def test_navigation_history_is_used(mock_pro, small_graph, fan):
    ...
    parse_navigation_request("take me to G", fan, history, small_graph)
    prompt = mock_pro.return_value.generate_content.call_args.args[0]
    assert "I'm at gate A" in prompt
    assert "OK, where to?" in prompt


def test_navigation_empty_query_short_circuits(small_graph, fan):
    """Empty query must not touch Gemini."""
    with patch("app.agents.intent.pro") as mock_pro:
        result = parse_navigation_request("   ", fan, [], small_graph)
        assert isinstance(result, UnresolvableRequest)
        mock_pro.assert_not_called()


@patch("app.agents.intent.pro")
def test_navigation_unknown_zone_id_raises(mock_pro, small_graph, fan):
...
@patch("app.agents.intent.pro")
def test_navigation_malformed_json_raises(mock_pro, small_graph, fan):
...
@patch("app.agents.intent.pro")
def test_navigation_unknown_type_raises(mock_pro, small_graph, fan):
...
@patch("app.agents.intent.pro")
def test_navigation_missing_type_raises(mock_pro, small_graph, fan):
...
@patch("app.agents.intent.pro")
def test_navigation_validation_failure_raises(mock_pro, small_graph, fan):
...
@patch("app.agents.intent.pro")
def test_profile_complete_with_flags(mock_pro):
    mock_pro.return_value = _mock_client(
        json.dumps(
            {
    ...
    result = extract_profile("I'm in 128, my mother uses a wheelchair")
...
@patch("app.agents.intent.pro")
def test_profile_complete_no_accessibility_is_still_complete(mock_pro):
    ...
    result = extract_profile("Estoy en la sección 305")
    assert isinstance(result, ProfileComplete)
...
@patch("app.agents.intent.pro")
def test_profile_incomplete_missing_seat(mock_pro):
    mock_pro.return_value = _mock_client(
        json.dumps(
            {
...
@patch("app.agents.intent.pro")
def test_profile_malformed_json_raises(mock_pro):
...
@patch("app.agents.intent.pro")
def test_profile_unknown_type_raises(mock_pro):
    mock_pro.return_value = _mock_client(json.dumps({"type": "banana"}))
    with pytest.raises(GeminiServiceError, match="unknown type"):
        extract_profile("hello")
...
@patch("app.agents.intent.pro")
def test_profile_missing_type_raises(mock_pro):
...
@patch("app.agents.intent.pro")
def test_profile_validation_failure_raises(mock_pro):
    mock_pro.return_value = _mock_client(
